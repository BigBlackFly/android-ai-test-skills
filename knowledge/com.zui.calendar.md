...
- 数据、权限与无课程表前提的准备统一按 [执行前设备环境准备](../docs/执行前设备环境准备.md)。
...
> 以上是清理后的界面现象；是否清理及如何建立前提，统一按
> [执行前设备环境准备](../docs/执行前设备环境准备.md) 判断。
...
准备顺序见 [执行前设备环境准备](../docs/执行前设备环境准备.md)：先完成清理判断，再建立备份前提。
若用例明确验证"清数据后恢复"，必须先把备份目录拷出来、clear 后再拷回去。
...
权限前提按 [执行前设备环境准备](../docs/执行前设备环境准备.md) 建立；本节只说明导入时两类弹窗的顺序。

## 坑-图库导入解析

- 解析需联网（约 20s），**等待窗口给到 60s**；低于 20s 会在解析中途误判超时
- 按 [内置测试媒体](../media-resources/README.md) 选择已预置的课程表素材；
  用例另需图片时，在环境初始化后准备，并确认媒体索引可发现它，再打开 PhotoPicker。
...
入口仅在**图库导入确认页** `TempConfirmTimetableActivity`，`EditTimetableActivity` 编辑页和课程时间设置页没有该入口。默认 `5分钟前`。
导航见 [路径图的图库导入流程](paths/com.zui.calendar.md#图库导入流程)。
...
**通用机制（默认同意、权限意图、rid 匹配）见 `_system.md`「权限弹窗-响应机制」
—— 那是跨 App 系统行为，本卡不复制。**
