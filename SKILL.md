...
> ⚠️ **不要重复 observe**：`act` 的返回里已经有截图 + dump + nodes。
> 只看画面**不用**再 observe；需要**页面文字**时用 `act --full`，不要额外起一次 observe。
> 实测一次多余的 observe = 8 秒工具时间 + 约 15 秒思考等待。

单独 `observe.py` 只在：会话第一步、上一个 act 找不到目标时用。
